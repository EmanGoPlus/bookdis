...
export default function HomeScreen() {
  ...
  useEffect(() => {
    ...
    socket.on("promo-created", (newPromo) => {
      console.log("New promo received:", newPromo);
  ...
  const claimPromo = async (promoId) => {
    try {
      const res = await axios.post(`${API_BASE_URL}/api/user/customer/claim-promo/${promoId}`);
      if (res.data.success) setClaimedQr(res.data.data.qrCode);
    } catch (err) {
  ...
  return (
    <View style={{ flex: 1, padding: 20 }}>
      <Text style={{ fontSize: 20, fontWeight: "bold", marginBottom: 10 }}>Available Promos</Text>
      <FlatList
        data={promos}
        keyExtractor={(item) => item.id.toString()}
        renderItem={({ item }) => (
          <View style={{ padding: 15, marginBottom: 10, backgroundColor: "#f2f2f2", borderRadius: 8 }}>
            <Text style={{ fontSize: 16, fontWeight: "bold" }}>{item.title}</Text>
            <Text>{item.description}</Text>
            <Text>{item.discountType}: {item.discountValue}</Text>
            <TouchableOpacity
              style={{ marginTop: 10, backgroundColor: "blue", padding: 10, borderRadius: 5 }}
              onPress={() => claimPromo(item.id)}
            >
              <Text style={{ color: "white", textAlign: "center" }}>Claim Promo</Text>
            </TouchableOpacity>
          </View>
        )}
      />
