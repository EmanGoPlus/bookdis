...
const businessController = {
  ...
  async uploadDocuments(request, reply) {
    try {
      ...
      for await (const part of parts) {
        console.log(
          `📝 Processing part: ${part.fieldname}, isFile: ${!!part.file}`
        );
      ...
      const documentRecord = await businessModel.uploadDocuments(
        businessId,
        docs
      );
  ...
  async verifyDocument(req, res) {
    try {
      ...
      const updated = await businessModel.setStatus(
        documentId,
        isVerified,
        rejectedReason
      );
      return res
        .status(200)
        .json({ message: "Document status updated", data: updated });
    } catch (error) {
